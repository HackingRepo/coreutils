...
pub fn uumain(args: impl uucore::Args) -> UResult<()> {
    ...
    if files.len() > 1 {
        return Err(uucore::error::USimpleError::new(
            ...
            format!(
                "{}\n{}",
                translate!("tsort-error-extra-operand", "operand" => files[1].to_string_lossy().quote()),
                translate!("tsort-try-help", "command_name" => uucore::util_name())
            ),
        ));
    }
    ...
    Command::new(uucore::util_name())
        ...
        .arg(
            Arg::new(options::WARN)
                .short('w')
                .long(options::WARN)
                .help(translate!("tsort-option-warn"))
